...
public class UnmuteCMD implements CommandExecutor {
	...
	public boolean onCommand(CommandSender s, Command c, String arg2, String[] a) {
		if (!(s instanceof Player)) {
			if (a.length == 0) {
				s.sendMessage("§cERRO! Use /unmute (Jogador).");
				return true;
			}
			Jogador j = new Jogador(a[0]);
			if (!j.hasPunish() || j.getPunish().getPunishType() != PunishType.MUTE || !j.getPunish().isAtiva()) {
				s.sendMessage("§c§lERRO! §cEsse jogador não está mutado!");
				return true;
			}
			Main.manager.Unmute(j.getPunish(), true, "Console");
			return true;
		}
		Player p = (Player)s;
		if (a.length != 1) {
			p.sendMessage("§c§lERRO! §cUse, /unmute (Jogador).");
			return true;
		}
		Jogador j = new Jogador(a[0]);
		if (!j.hasPunish() || j.getPunish().getPunishType() != PunishType.MUTE || !j.getPunish().isAtiva()) {
			p.sendMessage("§c§lERRO! §cEsse jogador não está mutado!");
			return true;
		}
		Main.manager.Unmute(j.getPunish(), true, p.getName());
		return true;
	}
}
